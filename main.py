...
class App(tk.Frame):
    def __init__(self, master, player_dets, line_annos, video):
        tk.Frame.__init__(self, master)
        self.pack(side=tk.LEFT)
        self.master.title("Rugby Tracker")
        self.master.resizable(False, False)
        self.tracker = Tracker(video, player_dets, line_annos)
        ...
        self.lines_but = tk.Button(self.options_menu, text='Footage Lines [Off]', font='Helvetica 14', command=self.toggle_lines)
        self.lines_but.pack(fill='both', anchor='w')
        self.footage_but = tk.Button(self.options_menu, text='Footage Detections [Off]', font='Helvetica 14', command=self.toggle_footage_detections)
        self.footage_but.pack(fill='both', anchor='w')
        self.homog_but = tk.Button(self.options_menu, text='Homography translations [Off]', font='Helvetica 14', command=self.toggle_homographies)
        self.homog_but.pack(fill='both', anchor='w')
        self.filter_but = tk.Button(self.options_menu, text='Particle Filters [On]', font='Helvetica 14', command=self.toggle_particles)
    ...
    def update_frames(self):
        if self.should_play:
            ...
            self.players_and_frames.configure(text='Number of active filters: '+str(n_filters)+'\nFrame: '+str(frame_no))
...
if  __name__ == '__main__':
    root = tk.Tk()
    with h5py.File('line_annotations.h5', 'r') as line_annos:
        with h5py.File('player_annotations.h5', 'r') as player_dets:
            app = App(root, player_dets, line_annos, 'footage.mp4')
            app.mainloop()
